...
class PosConfig(models.Model):
    ...
    def _default_warehouse_id(self):
        return self.env['stock.warehouse'].search(self.env['stock.warehouse'].check_company_domain(), limit=1).id

    def _default_picking_type_id(self):
        return self.env['stock.warehouse'].search(self.env['stock.warehouse'].check_company_domain(), limit=1).pos_type_id.id

    def _default_sale_journal(self):
        return self.env['account.journal'].search([
            *self.env['account.journal'].check_company_domain(),
    ...
    def _default_payment_methods(self):
        ...
        """
        domain = [
            *self.env['pos.payment.method'].check_company_domain(),
    ...
    def assign_payment_journals(self, company):
        for pos_config in self:
            ...
            cash_journal = self.env['account.journal'].search([
                *self.env['account.journal'].check_company_domain(company),
                ('type', '=', 'cash'),
            ], limit=1)
    ...
    def _get_availlable_product_domain(self):
        domain = [
            *self.env['product.product'].check_company_domain(self.company_id),
