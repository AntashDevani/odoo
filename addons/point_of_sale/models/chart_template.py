...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _load(self, template_code, company, install_demo):
        ...
        if not reload_template:
            self.env['pos.payment.method'].search(self.env['pos.payment.method'].check_company_domain(company)).unlink()
            self.env["pos.config"].search(self.env['pos.config'].check_company_domain(company)).write({
                'journal_id': False,
