...
class ResCompany(models.Model):
    ...
    def _compute_multi_vat_foreign_country(self):
        company_to_foreign_vat_country = {
            ...
            for company, country_ids in self.env['account.fiscal.position']._read_group(
                domain=[
                    *self.env['account.fiscal.position'].check_company_domain(self),
                    ('foreign_vat', '!=', False),
                ],
                groupby=['company_id'],
    ...
    def _compute_account_enabled_tax_country_ids(self):
        for record in self:
            ...
            foreign_vat_fpos = self.env['account.fiscal.position'].search([
                *self.env['account.fiscal.position'].check_company_domain(record),
                ('foreign_vat', '!=', False)
            ])
    ...
    def reflect_code_prefix_change(self, old_code, new_code):
        ...
        accounts = self.env['account.account'].search([
            *self.env['account.account'].check_company_domain(self),
            ('code', '=like', old_code + '%'),
    ...
    def create_op_move_if_non_existant(self):
        ...
        if not self.account_opening_move_id:
            default_journal = self.env['account.journal'].search([
                *self.env['account.journal'].check_company_domain(self),
                ('type', '=', 'general')
            ], limit=1)
    ...
    def get_chart_of_accounts_or_fail(self):
        account = self.env['account.account'].search(self.env['account.account'].check_company_domain(self), limit=1)
        if len(account) == 0:
    ...
    def _check_hash_integrity(self):
        ...
        journals = self.env['account.journal'].search(self.env['account.journal'].check_company_domain(self))
        results_by_journal = {
            'results': [],
