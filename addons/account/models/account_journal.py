...
class AccountJournalGroup(models.Model):
    ...
    check_company_domain = models.Model._check_company_domain_parent_of
...
class AccountJournal(models.Model):
    ...
    def copy(self, default=None):
        ...
        read_codes = self.env['account.journal'].with_context(active_test=False).search_read(
            self.env['account.journal'].check_company_domain(self.company_id),
            ['code'],
        )
    ...
    def _fill_missing_values(self, vals):
        ...
        random_account = self.env['account.account'].search(
            self.env['account.account'].check_company_domain(company),
            limit=1,
        )
