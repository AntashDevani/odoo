...
class AccountPaymentMethodLine(models.Model):
    ...
    def _compute_payment_provider_id(self):
        providers = self.env['payment.provider'].sudo().search([
            *self.env['payment.provider'].check_company_domain(self.journal_id.company_id),
    ...
    def _get_payment_method_domain(self, code):
        ...
        if unique:
            ...
            if company_ids:
                domain = expression.AND([domain, self.env['payment.provider'].check_company_domain(company_ids)])
