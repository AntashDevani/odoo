...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _parse_companies(self, companies):
        if companies is None:
            return self.env.company.ids
        elif isinstance(companies, BaseModel):
            return companies.ids
        elif isinstance(companies, (list, tuple)):
            return companies
        return [companies]

    def _check_company_domain_parent_of(self, companies=None):
        return ['|', ('company_id', '=', False), ('company_id', 'parent_of', self._parse_companies(companies))]

    def check_company_domain(self, companies=None):
        return ['|', ('company_id', '=', False), ('company_id', 'in', self._parse_companies(companies))]
    ...
    def _check_company(self, fnames=None):
        ...
        for record in self:
            ...
            for name in regular_fields:
                ...
                if corecord and ('company_id' in corecord._fields or corecord._name == 'res.company'):
                    domain = corecord.check_company_domain(company)
                    if company and domain and not corecord.filtered_domain(domain):
